...
static void crsf_decode_channels(const uint8_t *p_payload) {
  ...
  raw_ch[2]  = (p_payload[2]  >> 6 | p_payload[3]  << 2 | p_payload[4] << 10) & 0x07FF;
  raw_ch[3]  = (p_payload[4]  >> 1 | p_payload[5]  << 7) & 0x07FF;
  raw_ch[4]  = (p_payload[5]  >> 4 | p_payload[6]  << 4) & 0x07FF;
  raw_ch[5]  = (p_payload[6]  >> 7 | p_payload[7]  << 1 | p_payload[8] << 9) & 0x07FF;
  ...
  raw_ch[10] = (p_payload[13] >> 6 | p_payload[14] << 2 | p_payload[15] << 10) & 0x07FF;
  raw_ch[11] = (p_payload[15] >> 1 | p_payload[16] << 7) & 0x07FF;
  raw_ch[12] = (p_payload[16] >> 4 | p_payload[17] << 4) & 0x07FF;
  raw_ch[13] = (p_payload[17] >> 7 | p_payload[18] << 1 | p_payload[19] << 9) & 0x07FF;
  ...
  for (uint8_t i = 0; i < CRSF_NUM_CHANNELS; i++) {
    ...
    // Công thức scale tuyến tính: 988 + (raw - 172) * (2012 - 988) / (1811 - 172)
    // Rút gọn thành: 988 + (raw - 172) * 1024 / 1639
...
void crsfInit() {
  ...
  for (uint8_t i = 0; i < CRSF_NUM_CHANNELS; i++) {
    crsf_channels[i] = (i == 2) ? 1000 : CRSF_CHANNEL_MID_US; // Kênh 2 là Throttle = 1000us
  }

  // Khởi tạo Serial2 chuẩn phần cứng kết nối bộ thu ELRS (PA2 TX / PA3 RX) ở 420000 baud
  // STM32 Arduino Core tự động cấu hình PA2/PA3 khi khởi tạo Serial2
      ...
      case STATE_WAIT_PAYLOAD:
        ...
        if (write_index == frame_len) {
          ...
          if (calculated_crc == received_crc) {
            ...
            if (packet_type == CRSF_FRAMETYPE_RC_CHANNELS_PACKED && frame_len == 25) {
              crsf_decode_channels(&rx_buffer[1]); // Giải nén payload kênh bắt đầu từ byte thứ 2
              last_crsf_packet_time = millis();    // Cập nhật mốc thời gian nhận tin
            } 
            else if (packet_type == CRSF_FRAMETYPE_LINK_STATISTICS && frame_len == 11) {
              crsf_lq = rx_buffer[3]; // Vị trí Link Quality trong payload Link Stats
...
uint16_t crsfGetChannel(uint8_t ch) {
  if (ch >= CRSF_NUM_CHANNELS) return CRSF_CHANNEL_MID_US;
  
  // Giới hạn giá trị trả về trong khoảng an toàn tuyệt đối
  uint16_t val = crsf_channels[ch];
  if (val < CRSF_CHANNEL_MIN_US) val = CRSF_CHANNEL_MIN_US;
  if (val > CRSF_CHANNEL_MAX_US) val = CRSF_CHANNEL_MAX_US;
  return val;
}

bool crsfIsLinkActive() {
  if (last_crsf_packet_time == 0) return false;
...
uint8_t crsfGetLq() {
  if (!crsfIsLinkActive()) return 0;
  return crsf_lq;
}

int8_t crsfGetRssi() {
  if (!crsfIsLinkActive()) return -120;
  return crsf_rssi;
}
