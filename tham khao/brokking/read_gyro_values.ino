void read_gyro_values(void) {
  ...
  while (data != 'q') {                                                                   //Stay in this loop until the data variable data holds a q.
    ...
    if (data == 'c') {                                                                    //If the user requested a 'c'.
      if (cal_int != 2000) {                                                              //Chỉ calibrate nếu chưa calibrate (cal_int chưa bằng 2000). Lần đầu vào, cal_int = 0 → vào đây. Calibrate xong cal_int sẽ = 2000
        ...
        gyro_axis_cal[3] = 0;                                                             //Reset calibration variables for next calibration. Xóa 3 ô tích lũy (cho trục roll/pitch/yaw) về 0, chuẩn bị cộng dồn.
        Serial.print("Calibrating the gyro");
        //Let's take multiple gyro data samples so we can determine the average gyro offset (calibration).
        for (cal_int = 0; cal_int < 2000 ; cal_int ++) {                                  //Take 2000 readings for calibration.
          if (cal_int % 125 == 0) {                                                       //After every 125 readings.
            digitalWrite(PB3, !digitalRead(PB3));                                         //Cứ mỗi 125 vòng thì đảo trạng thái đèn
            Serial.print(".");                                                            //in một dấu chấm. → Tạo hiệu ứng "...." chạy dần, báo còn đang làm việc (không bị treo).
          }
          gyro_signalen();                                                                //Read the gyro output. gọi hàm đọc cảm biến (đọc gyro + accel về các biến gyro_axis[], acc_axis[])
        ...
        Serial.println(".");
        //Now that we have 2000 measures, we need to devide by 2000 to get the average gyro offset. Tính trung bình
      ...
      gyro_signalen();                                                                    //Read the gyro output. Đọc cảm biến lần nữa (gyro_signalen), rồi in 3 trục gyro ra màn hình:
      Serial.print("Gyro_x = ");
    ...
    else {                                                                                //If the user requested a 'd'. Nhánh cho lệnh d: đọc cảm biến rồi in 3 trục accel. Nhánh này không calibrate
