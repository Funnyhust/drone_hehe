void handler_channel_1(void) {                           //This function is called when channel 1 is captured.
  if (0b1 & GPIOA_BASE->IDR  >> 0) {                     //If the receiver channel 1 input pulse on A0 is high. // (A) Chân đang HIGH?
    channel_1_start = TIMER2_BASE->CCR1;                 //Record the start time of the pulse. Ghi mốc bắt đầu xung 
    TIMER2_BASE->CCER |= TIMER_CCER_CC1P;                //Change the input capture mode to the falling edge of the pulse.Chuyển sang bắt mép XUỐNG
  }
  else {                                                 //If the receiver channel 1 input pulse on A0 is low. Chân đang LOW?
    channel_1 = TIMER2_BASE->CCR1 - channel_1_start;     //Calculate the total pulse time. Tính độ rộng xung
    if (channel_1 < 0)channel_1 += 0xFFFF;               //If the timer has rolled over a correction is needed. Bù khi timer tràn
    TIMER2_BASE->CCER &= ~TIMER_CCER_CC1P;               //Change the input capture mode to the rising edge of the pulse.Chuyển lại bắt mép LÊN
  }
}
...
//handler_channel_1PA0 (>>0)Timer2CCR1channel_1
//handler_channel_2PA1 (>>1)Timer2CCR2channel_2
//andler_channel_3PA2 (>>2)Timer2CCR3channel_3
//handler_channel_4PA3 (>>3)Timer2CCR4channel_4
