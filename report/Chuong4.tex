...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[
        ...
        \node[thread] (gui_thread) at (8.4,0) {Luồng giao diện GUI\\(Vòng lặp chính OpenCV)};
        ...
        \node[align=center, font=\fontsize{7.5pt}{8.5pt}\selectfont] at (10.7, 1.75) {Hàng đợi Thread-safe\\(Khung hình, Khung xương,\\Danger Score)};
            
        \draw[signal] (serial_thread.north) -- ++(0,0.25) -| ($(gui_thread.north west)+(0.4,0)$);
        \node[align=center, font=\fontsize{7.5pt}{8.5pt}\selectfont] at (5.8, 1.25) {Luồng truyền tin\\(Góc nghiêng,\\Điện áp pin)};
...
Các luồng xử lý chạy song song độc lập tương tác qua cơ chế hàng đợi (Thread-safe Queue) và chia sẻ luồng dữ liệu an toàn:
\begin{enumerate}
    \item \textbf{Luồng chính (GUI Loop)}: Đảm nhận vòng lặp chính của OpenCV, xử lý vẽ giao diện lên khung hình video và nhận sự kiện phím bấm qua \texttt{cv2.waitKey}, đồng thời khởi tạo cửa sổ nhật ký sự kiện Tkinter khi cần.
    ...
    \item \textbf{Luồng suy luận AI (Inference Thread)}: Lấy khung hình từ Frame Queue, đẩy qua bộ Detector tích hợp 3 mô hình YOLO (YOLOv8-Rescue, YOLOv8-Posture, YOLOv8-Pose), thực hiện thuật toán dung hợp suy luận và gửi kết quả khung hình đã vẽ bounding box cùng tọa độ keypoints về luồng chính hiển thị.
