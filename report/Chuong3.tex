...
Tổng khối lượng dữ liệu phục vụ bài toán phân loại và phát hiện trạng thái cứu hộ là 82.623 hình ảnh. Tập dữ liệu huấn luyện bao gồm 71.651 ảnh cung cấp các đặc trưng thị giác cốt lõi để mạng nơ-ron tối ưu hóa trọng số qua các vòng lặp. Tập kiểm thử (Validation) bao gồm 10.972 ảnh độc lập được tách biệt hoàn toàn khỏi quá trình cập nhật trọng số để đánh giá khách quan hiệu năng mô hình. Cấu trúc phân vùng chi tiết của tập dữ liệu cứu hộ được tổng hợp trong Bảng \ref{tab:dataset_rescue}.
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[
        ...
        % Column 2: Detectors
        \node[block, text width=3.4cm] (rescue) at (4.0, 2.0) {\textbf{YOLOv8-Rescue}\\Phát hiện người/nạn nhân};
        \node[block, text width=3.4cm] (posture) at (4.0, 0) {\textbf{YOLOv8-Posture}\\Phân loại tư thế};
        \node[block, text width=3.4cm] (pose) at (4.0, -2.0) {\textbf{YOLOv8-Pose}\\Ước lượng 17 keypoints};

        % Column 3: Fusion & Danger Score
        \node[block, text width=5.2cm, minimum height=1.8cm] (danger_score) at (11.5, 0) {
        ...
        % Column 4: Classifier
        \node[block, fill=gray!5, text width=2.2cm] (classifier) at (17.0, 0) {\textbf{Bộ phân loại}\\\textbf{Ngưỡng điểm DS}};

        % Column 5: Action Outputs
        \node[block, fill=gray!15, text width=4.8cm] (danger_out) at (22.5, 2.0) {
            \textbf{Cứu trợ khẩn cấp (DS $\ge$ 85)}\\
            Khung bao Đỏ (Giao diện), Phát âm báo,\\
            Chụp ảnh lưu trữ \& Xuất báo cáo GPS
        };

        \node[block, text width=4.8cm] (suspicious_out) at (22.5, 0) {
            \textbf{Trạng thái Nghi ngờ ($55 \le$ DS $<$ 85)}\\
            Khung bao Vàng (Giao diện), Tăng tần số trích lọc
        };

        \node[block, text width=4.8cm] (normal_out) at (22.5, -2.0) {
        ...
        \draw[arrow] (frame.east) -- (1.8, 0) -- (1.8, 2.0) -- (rescue.west);
        \draw[arrow] (frame.east) -- (posture.west);
        \draw[arrow] (frame.east) -- (1.8, 0) -- (1.8, -2.0) -- (pose.west);

        % Column 2 to Column 3 (danger_score)
        \draw[arrow] (rescue.east) -- node[above, midway, font=\fontsize{7.5pt}{7.5pt}\selectfont] {$S_{YOLO\_Pose}$} (7.8, 2.0) -- (7.8, 0.45) -- ([yshift=0.45cm]danger_score.west);
        \draw[arrow] (posture.east) -- node[above, midway, font=\fontsize{7.5pt}{7.5pt}\selectfont] {$S_{Posture}$} (danger_score.west);
        \draw[arrow] (pose.east) -- node[above, midway, font=\fontsize{7.5pt}{7.5pt}\selectfont] {$S_{Motion}, S_{Gesture}$} (7.8, -2.0) -- (7.8, -0.45) -- ([yshift=-0.45cm]danger_score.west);

        % Connect danger score to classifier
        \draw[arrow] (danger_score.east) -- (classifier.west);

        % Connect classifier to outputs
        \draw[arrow] (classifier.east) -- (19.1, 0) -- (19.1, 2.0) -- (danger_out.west);
        \draw[arrow] (classifier.east) -- (suspicious_out.west);
        \draw[arrow] (classifier.east) -- (19.1, 0) -- (19.1, -2.0) -- (normal_out.west);
...
Để tối ưu hóa độ chính xác và giảm thiểu tỷ lệ báo động giả (False Alarm) từ các mô hình nhận diện đơn lẻ, hệ thống áp dụng thuật toán dung hợp quyết định đa mô hình (Decision Fusion). Thay vì tin cậy vào kết quả của một mô hình học sâu duy nhất, trạm mặt đất tích hợp dữ liệu suy luận đồng thời từ mô hình YOLOv8-Rescue (phân loại người gặp nạn), YOLOv8-Posture (nhận diện tư thế) và YOLOv8-Pose (trích xuất 17 điểm khớp xương để đánh giá chuyển động và cử chỉ vẫy tay).

Thuật toán dung hợp được hiện thực hóa thông qua việc tích lũy các nguồn thông tin độc lập này thành một chỉ số định lượng duy nhất gọi là Điểm nguy cấp (Danger Score - $DS$) chạy từ 0 đến 100 điểm. Chi tiết các quy tắc tính toán điểm số và dung hợp logic ngữ cảnh được trình bày trong tiểu mục tiếp theo.
...
\begin{itemize}
    ...
    Đưa khung hình qua bộ điều phối \texttt{detector.py} thực thi song song 3 mô hình học sâu: YOLOv8-Pose trích xuất 17 khớp xương và theo dõi ID đối tượng bằng ByteTrack; YOLO-Rescue nhận dạng người/nạn nhân; YOLO-Posture nhận dạng các tư thế cơ bản. Kết hợp phân tích hình học keypoints để đánh giá nhanh tư thế (nằm, gục) và cử chỉ động (giơ tay, vẫy tay cầu cứu). Sau đó, thực hiện so khớp bounding box bằng chỉ số IoU $\ge 0.20$ và dung hợp xác suất theo công thức tích lũy độc lập để đưa ra kết luận phân loại nạn nhân tối ưu.
