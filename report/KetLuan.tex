...
\begin{enumerate}
    ...
    \item \textbf{Về phần mềm trạm mặt đất và thuật toán AI}: Phát triển thành công phần mềm trạm mặt đất đa luồng OpenCV kết nối luồng video FPV thời gian thực. Đề xuất cơ chế dung hợp quyết định (Decision Fusion) đa luồng kết hợp 3 mô hình YOLO (YOLOv8-Rescue, YOLOv8-Posture, YOLOv8-Pose) đạt độ chính xác F1-score nhận dạng nạn nhân cứu hộ lên tới 88.0\% ở tốc độ xử lý thời gian thực 27.8 FPS. Tích hợp thành công module tự động ghi nhật ký sự kiện cứu hộ và sinh báo cáo HTML chứa tọa độ GPS và hình ảnh trực quan.
\end{enumerate}
